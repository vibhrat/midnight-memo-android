...
const Search = ({
  ...
  const searchResults = useMemo(() => {
    ...
    // Search notes
    notes.forEach(note => {
      if (note.title.toLowerCase().includes(query.toLowerCase()) || stripHtmlTags(note.content).toLowerCase().includes(query.toLowerCase()) || note.tag.toLowerCase().includes(query.toLowerCase())) {
        results.push({
          id: note.id,
          type: 'note',
          title: note.title,
    ...
    lists.forEach(list => {
      ...
      if (list.title.toLowerCase().includes(query.toLowerCase()) || itemsText.toLowerCase().includes(query.toLowerCase())) {
        results.push({
          id: list.id,
          type: 'list',
          title: list.title,
  ...
  return <div className="min-h-screen bg-[#000000]">
      <div className="max-w-2xl mx-auto p-4 pb-20 py-[72px]">
        <div className="flex items-center gap-4 mb-6">
          ...
          <input placeholder="Search notes, lists, and passwords..." value={query} onChange={e => setQuery(e.target.value)} className="flex-1 bg-transparent border-0 border-b border-[#9B9B9B] text-[#DBDBDB] placeholder:text-[#9B9B9B] px-0 py-2 focus:outline-none focus:border-[#DBDBDB] rounded-none" autoFocus />
        </div>

        <div className="space-y-3">
          {searchResults.length === 0 && query.trim() && <div className="text-center py-12 text-[#9B9B9B]">
              No results found for "{query}"
            </div>}

          {searchResults.map(result => <div key={`${result.type}-${result.id}`} className="p-4 bg-[#181818] rounded-lg cursor-pointer hover:bg-[#2A2A2A] transition-colors" onClick={() => handleResultClick(result)}>
              ...
              {result.content && <p className="text-sm text-[#9B9B9B] mb-2">
              ...
              <div className="flex items-center justify-between">
                ...
                {result.itemCount !== undefined && <span className="text-sm font-medium text-[#9B9B9B]">
