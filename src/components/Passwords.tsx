...
const Passwords = forwardRef<PasswordsRef, PasswordsProps>(({
  ...
  const filteredPasswords = passwords.filter(password => {
    ...
    return password.title.toLowerCase().includes(searchText) || password.username.toLowerCase().includes(searchText) || password.url?.toLowerCase().includes(searchText);
  });
  return <div className="min-h-screen bg-[#000000] relative">
      ...
      <div className="max-w-2xl mx-auto p-4 pb-20 relative z-10">
        ...
        <div className="space-y-4">
          {filteredPasswords.map(password => <Card key={password.id} className="cursor-pointer hover:bg-[#2A2A2A] transition-colors bg-[#181818] border-0 rounded-lg" onClick={() => handleCardClick(password.id)}>
          ...
          {passwords.length === 0 && <EmptyStateIllustration />}
        </div>
      </div>
    </div>;
});
