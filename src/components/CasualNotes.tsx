...
const CasualNotes = forwardRef<CasualNotesRef, CasualNotesProps>(({
  ...
  const availableTags = allTags.filter(tag => notes.some(note => note.tag === tag));
  ...
  return <div className="min-h-screen bg-[#000000] relative">
      ...
      <div className="max-w-2xl mx-auto p-4 pb-20 relative z-10 py-[64px]">
        <NotesHeader onSearchClick={onSearchClick} onMenuClick={onMenuClick} onImportClick={() => setShowImportDialog(true)} />

        <TagFilter availableTags={availableTags} selectedTag={selectedTag} onTagSelect={setSelectedTag} />

        <NotesList notes={notes} filteredNotes={filteredNotes} selectedTag={selectedTag} onNoteClick={handleCardClick} />

        <ImportDialog isOpen={showImportDialog} onClose={() => setShowImportDialog(false)} type="note" />
      </div>
    </div>;
});
