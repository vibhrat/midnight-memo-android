...
const PasswordDetail = ({ passwordId, onBack }: PasswordDetailProps) => {
    ...
    return (
      <div className="min-h-screen bg-[#000000] flex items-center justify-center">
        <div className="text-center">
          ...
          <button 
            onClick={onBack} 
            className="mt-4 px-4 py-2 bg-[#DBDBDB] text-[#000000] rounded"
          >
            Go Back
          </button>
        </div>
  ...
  const confirmFieldDelete = () => {
    if (fieldToDelete) {
      const updatedFields = (editablePassword.fields || []).filter(field => field.id !== fieldToDelete);
      autoSave({ fields: updatedFields });
    ...
    <div className="min-h-screen bg-[#000000]">
      <div className="max-w-2xl mx-auto p-4 pb-20 py-[9px]">
        ...
        <div className="flex justify-between items-center mb-6">
          <button 
            onClick={onBack} 
            className="p-2 border border-[#9B9B9B] rounded-lg hover:bg-[#181818] py-[8px] my-[64px]"
          >
      ...
      <DeleteConfirmDialog 
        ...
        onClose={() => setShowFieldDeleteDialog(false)} 
